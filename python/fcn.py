...
# 全卷积网络：将改进的VGG网络（FC改为FCN），再加入5个上采样层，最后进行逐像素分类
...
class VGGNet(VGG):
    def __init__(self, pretrained=True, model='vgg16', requires_grad=True, remove_fc=True, show_params=False):
        ...
        # 如果是预训练，则加载模型已经预训练好的权重
        if pretrained:
            # vgg预训练模型：models.vgg(pretrained=True)
            # 获取模型参数：models.vgg(pretrained=True).state_dict()
            # 加载参数：self.load_state_dict(models.vgg(pretrained=True).state_dict())
            exec("self.load_state_dict(models.%s(pretrained=True).state_dict())" % model)
        if not requires_grad:
            # 冻结这一层的网络参数（关闭自动求导功能），不训练这一层
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            in_channels = v
    # *号加在了是实参上，代表的是将输入迭代器拆成一个个元素
    return nn.Sequential(*layers)


if __name__ == "__main__":
    batch_size, n_class, h, w = 2, 20, 160, 160
    ...
    # fcn = FCNs(pretrained_net=vgg, n_class=20)
    # fcn = fcn.to(device)
    # summary(fcn, (3, 160, 160))